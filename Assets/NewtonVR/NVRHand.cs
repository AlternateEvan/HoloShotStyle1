...
namespace NewtonVR
{
    public class NVRHand : MonoBehaviour
    {
        ...
        //TODO ALTSPACE: magic get/set
		public SteamVR_Controller.Device Controller { get; private set; }

		//TODO ALTSPACE: the haptic pulse value to use for this frame
		private ushort hapticPulseDuration = 0;
		...
		protected int RotationEstimationSamples = 10;
        ...
        public bool IsHovering
        {
            get
            {
				//TODO ALTSPACE: check if kvp.Key is null (someone destroyed the object!)
				return CurrentlyHoveringOver.Any(kvp => kvp.Value.Count > 0 && kvp.Key != null);
			}
		}
		...
		public void TriggerHapticPulse(ushort durationMicroSec = 500, EVRButtonId buttonId = EVRButtonId.k_EButton_SteamVR_Touchpad)
		{
			if (Controller != null)
			{
				//TODO ALTSPACE: this allows us to apply haptic pulse from multiple sources and combine them
				if (durationMicroSec > hapticPulseDuration)
				{
					hapticPulseDuration = durationMicroSec;
				}
				Controller.TriggerHapticPulse(hapticPulseDuration, buttonId);
			}
		}

		//TODO ALTSPACE: reset the hapticPulseDuration every frame
	    void LateUpdate()
	    {
			hapticPulseDuration = 0;
	    }
		...
		//TODO ALTSPACE: make protected virtual
		protected virtual void PickupClosest()
        {
