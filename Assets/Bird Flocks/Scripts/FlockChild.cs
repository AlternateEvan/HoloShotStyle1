...
public class FlockChild:MonoBehaviour{
	...
	// AltspaceVR 
	// we use cached _modelAnimation throughout this file; the original author used GetComponent nearly everywhere
	public Animation _modelAnimation;
	// List of animator that are cached
    ...
    public void FindRequiredComponents(){
	    ...
	    if (_modelAnimation == null) _modelAnimation = _model.GetComponent<Animation>(); // AltspaceVR change
	    if (_animationStates == null) // AltspaceVR addition
	    {
    ...
    public void Flap(){
    	if(_move){
    	 	if(this._model != null) _modelAnimation.CrossFade(_spawner._flapAnimation, .5f);
    		_soar=false;
    		animationSpeed();
    ...
    public void Soar(){
    	if(_move){
			_modelAnimation.CrossFade(_spawner._soarAnimation, 1.5f);
    	   	_wayPoint= findWaypoint();
    	    _soar = true;
        }
    }
    ...
    {
		// AltspaceVR addition; use simple for loop to go over cached collection
		// saves on heap allocation
	    int numStates = _animationStates.Count;
        for (int stateIndex = 0; stateIndex < numStates; stateIndex++)
        {
	        AnimationState state = _animationStates[stateIndex];
    		if(!_dived && !_landing){
