...
public class LandingSpot:MonoBehaviour{
        ...
        if (_controller._flock.gameObject.activeInHierarchy && landing && (landingChild != null)) {
            ...
            if (distance < 5 && distance > .5f) {
                if(_controller._soarLand){
                	landingChild._modelAnimation.CrossFade(landingChild._spawner._soarAnimation, .5f);
                	if (distance < 2)
               	 		landingChild._modelAnimation.CrossFade(landingChild._spawner._flapAnimation, .5f);
                }
                ...
                if (distance < _controller._snapLandDistance && !_idle) {
                    _idle = true;
                    landingChild._modelAnimation.CrossFade(landingChild._spawner._idleAnimation, .55f); 
    ...
    public void InstantLand() {
        if (_controller._flock.gameObject.activeInHierarchy && (landingChild == null)) {
            ...
            if (fChild != null) {
                ...
                landingChild._modelAnimation.Play(landingChild._spawner._idleAnimation);
              if(_controller._autoDismountDelay.x > 0)  StartCoroutine(ReleaseFlockChild(_controller._autoDismountDelay.x, _controller._autoDismountDelay.y));
            } else if (_controller._autoCatchDelay.x > 0) {
                StartCoroutine(GetFlockChild(_controller._autoCatchDelay.x, _controller._autoCatchDelay.y));
            }
    ...
    public IEnumerator ReleaseFlockChild(float minDelay,float maxDelay) {
        ...
        if (_controller._flock.gameObject.activeInHierarchy && (landingChild != null)) {
            ...
            landingChild._modelAnimation.CrossFade(landingChild._spawner._flapAnimation, .2f);
